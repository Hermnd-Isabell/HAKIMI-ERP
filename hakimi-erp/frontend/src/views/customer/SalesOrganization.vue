  ...
  <div class="page">
    <div class="header-card">
      ...
      <div v-else class="data-card"><table class="dt">
        ...
        <tbody>
          <tr v-for="r in rows" :key="r.salesOrgId + '-' + r.distributionChannel + '-' + r.division" class="dr">
            <td class="mono">{{ r.salesOrgId }}</td><td>{{ r.description }}</td><td class="mono">{{ r.distributionChannel }}</td><td class="mono">{{ r.division }}</td><td>{{ r.currency }}</td><td>{{ r.country }}</td>
...
import { fetchSalesOrganizations } from '@/api'
...
onMounted(async () => {
  try {
    const data = await fetchSalesOrganizations({ pageSize: 50 })
