<template>
  <div class="page">
    <div class="header-card">
        ...
        <div class="hc-right">
          <button class="btn btn-primary" @click="router.push('/sales/inquiry/new')">
      ...
      <div class="filter-bar">
        <input type="text" class="form-input search-inq" v-model="filter.inquiryId" placeholder="Inquiry No." />
      ...
      <div class="data-card">
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr v-if="rows.length === 0 && !loading"><td colspan="7" style="text-align:center;padding:40px;color:#999;">No inquiries found.</td></tr>
            <tr v-if="loading"><td colspan="7" style="text-align:center;padding:40px;color:#999;">Loading...</td></tr>
          </tbody>
        </table>
...
async function fetchData() {
  loading.value = true
  try {
    const inqData = await fetchInquiries(filter)
    
    rows.value = (inqData.items || []).map((i: any) => ({
      id: i.inquiryId,
      no: i.inquiryId,
      cust: i.customerId,
      date: i.createdTime?.split('T')[0] || 'N/A',
      mat: i.items?.[0]?.materialId || 'N/A',
      qty: i.items?.[0]?.orderQuantity || '0',
      st: i.status,
...
function resetFilter() {
  Object.assign(filter, { inquiryId: '', customerName: '', status: '' })
  fetchData()
}
